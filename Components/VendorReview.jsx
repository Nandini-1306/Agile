...
const VendorReview = () => {
  ...
  const vendorID = localStorage.getItem('vendorID'); // Assuming you store vendorID in localStorage
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      ...
      try {
        const response = await axios.get(`http://localhost:3000/reviews/vendor?vendorID=${vendorID}`);
        setReviews(response.data);
      } catch (error) {
        console.error("Error fetching reviews: ", error);
        toast.error("Error fetching reviews. Please try again later.");
